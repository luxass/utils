...
import type { ElementOf, InferArguments, Prettify, RemoveIndexSignature } from "../src/types";
...
it("should infer the arguments type of a function", () => {
  expectTypeOf<InferArguments<(a: string, b: number) => void>>().toEqualTypeOf<
    [a: string, b: number]
  >();
});
...
it("should infer the arguments type of a function with optional and rest parameters", () => {
  expectTypeOf<InferArguments<(a: string, b?: number, ...c: boolean[]) => void>>().toEqualTypeOf<
